...
:::
```{bash}
gunzip -c DRR025089_1.fastq.gz | less -S | wc -l
```
...
Sí, tiene sentido guardar datos “crudos” descargados (FASTQ y referencia) en una carpeta tipo data/, para no mezclar código con datos y poder reutilizarlos desde distintos análisis.

- ¿Cómo podrías guardar los mensajes de `wget` en un archivo?
Para guardar los mensajes de wget (progreso, errores y logs) en un archivo, se usa la opción -o archivo.log ; esto redirige toda la salida informativa a ese archivo sin afectar la descarga del fichero principal.

- ¿De cuántas secuencias diferentes se compone este genoma de referencia?
En un FASTA, cada secuencia empieza con una línea de cabecera que comienza por >; por tanto, el número de secuencias es el número de líneas que empiezan por >.

grep -c '^>'referencia.fa  cuenta cuantos > hay en el archivo.
En el caso de Bradyrhizobium japonicum ACCC 15027 presente 3. Tiene un cromosoma circular y dos plásmidos. 
```{bash}
...
Los datos paired-end procesados por fastp muestran alta calidad inicial (Q20 80%, Q30 71%, GC 64%) y post-filtro excelente (91% reads retenidos: ~234K de 823K, Q20 90%, Q30 84%, ~50M bases), con remoción principal de baja calidad (8.5%), N excesivos (0.13%) y correcciones menores; bajo duplicado (2.7%), pico inserto 199 bp y sin adaptadores/dímeros significativos.
Conclusión: datos limpios, equilibrados (A/T~18%, C/G~32%) y listos para análisis downstream como transcriptómica bacteriana.
...
:::
Descargamos los nuevos archivos, o por comando de bash o través del enlace. En este caso, he elegido el conjunto DRR025081.
```{bash}
...
Creo el fastp.html para poder viasulizar los datos de forma correcta. 

```{bash}
if [ ! -e fastp_081.html ]; then
   # Si "fastp" está en la carpeta actual y no en el PATH, usa "./fastp"
...
```
Los datos paired-end procesados por fastp muestran buena calidad inicial (Q20 78%, Q30 68%, GC 64%) y post-filtro excelente (85% reads retenidos: ~82K de 424K, Q20 93%, Q30 88%, ~17M bases), con remoción principal de baja calidad (15%), N excesivos (0.14%) y correcciones menores; bajo duplicado (2.1%), pico inserto 128 bp y adaptadores mínimos (pre-trimmados).
Conclusión: Datos limpios, equilibrados (A/T~18%, C/G~32%) y listos para análisis downstream como transcriptómica bacteriana.
...
# borrar referencia después del alineamiento
#rm referencia.fa referencia.fa.* #con esto se borra referencia.fa
...
La función es samtools merge: 
samtools merge combinado.bam archivo1.bam archivo2.bam

4. ¿Cuánto espacio de disco ocupan los archivos FASTQ, el genoma de referencia y
   su índice y el archivo SAM?
```{bash}
du -h DRR025089_1_clean.fastq
du -h DRR025089_2_clean.fastq
du -h referencia.fa
du -h DRR025089.sam
```
Unidades en Mb: 81, 81, 9.7 y 192, respectivamente. 
...
El uso de tuberías en BASH permite ejecutar los programas de forma más rápida y ahorrar espacio en disco, ya que no se generan archivos intermedios como el SAM. Sin embargo, también tiene algunos inconvenientes: al no guardarse los resultados intermedios, es más difícil revisar los pasos del proceso o detectar errores. Además, si el pipeline se interrumpe por algún fallo del sistema o falta de recursos, es necesario repetir todo el proceso desde el principio, ya que no se puede continuar desde un paso intermedio.
...
Cuando un genoma tiene plásmidos, normalmente aparecen picos adicionales en la distribución de cobertura, porque los plásmidos suelen tener un número de copias distinto al cromosoma (muchas veces mayor), lo que genera coberturas diferentes.

Como en la gráfica solo aparece un pico principal y no hay otros picos claros, lo más probable es que la cepa secuenciada no contenga los dos plásmidos del genoma de referencia, o que su presencia sea muy baja y no genere un pico detectable en la distribución de cobertura.
